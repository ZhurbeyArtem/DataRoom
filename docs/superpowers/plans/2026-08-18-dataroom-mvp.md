...
## Задача 4: Вхід через Google — ЗНЯТО З ОБСЯГУ

Рішення від 23.08.2026: OAuth не робимо. Вхід лишається за email і паролем.
Кнопка Google, прапорець `VITE_GOOGLE_AUTH` і секція на екранах входу
прибрані з коду.
...
Задача 4 (Google OAuth) знята з обсягу. Задачу 20 (полірування) можна робити
паралельно з рештою, якщо є друга пара рук: вона торкається файлів,
створених у 13–19.
